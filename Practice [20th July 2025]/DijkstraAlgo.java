...
record ShortestPath (int sourceNode, int destinationNode, int distance) {

  @Override
  public String toString () {
    return "[Shortest distance from source node '" + sourceNode + "' to destination node '" + destinationNode + "' is '" + distance + "'.]";
  }
...
record Graph (Map<Integer, List<NodeWeightPair>> adjacentList, int numberOfNodes) {
  ...
  public List<ShortestPath> processShortestPath (int sourceNode) {
    ...
    distance[sourceNode] = 0; // The distance of source node from source node will always be zero(0).
	
    // Min heap implementation of priority queue to sort 'node' and 'distance from source node' pair in the 
    // ascending order of 'distance from source node'.
    Queue<NodeWeightPair> minHeap = new PriorityQueue<>((nwp1, nwp2) -> nwp1.edgeWeight() - nwp2.edgeWeight());
    minHeap.add(new NodeWeightPair(sourceNode, 0)); // The distance of source node from source node is always zero(0).
    ...
    List<ShortestPath> resultList = new ArrayList<>();
    for (int node = 0; node < this.numberOfNodes; node++)
      resultList.add(new ShortestPath(sourceNode, node, distance[node]));
...
public class DijkstraAlgo {
  ...
  public static void main (String[] args) {
    ...
    // Addning new edges to the graph ...
    for (List<Integer> edge : edgeList) 
      graph.addEdge(edge.get(0), edge.get(1), edge.get(2));
    
    // Computing the result ...
    List<ShortestPath> resultList = graph.processShortestPath(sourceNode);

    // Printing the result ...
    System.out.println("\nResult::");
    for (ShortestPath result : resultList)
