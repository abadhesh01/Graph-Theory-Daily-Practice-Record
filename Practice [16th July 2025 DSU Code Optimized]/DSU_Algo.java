...
record DSU (int[] parent, int[] rank) {
  ...
  public DSU (int numberOfNodes) {
    ...
    for (int node = 0; node < numberOfNodes; node++)
      parent[node] = node; // Every node is the parent of itself.	    
  }

  public int getUltimateParent (int node) {
    if (parent[node] != node)
      parent[node] = getUltimateParent(parent[node]); // return getUltimateParent(parent[node]);
    return node;    
  } 

  public void makeUnion (int node1, int node2) {
    int rootX = this.getUltimateParent(node1);
    int rootY = this.getUltimateParent(node2);
    if (rootX != rootY) {
...
public class DSU_Algo {
  ...
  public static void main (String[] args) {
    ...
    inputScanner.close();
    DSU dsu = new DSU(numberOfNodes); // Creating a new disjoint set union ...
    for (int node = 0; node < numberOfNodes; node++) {
